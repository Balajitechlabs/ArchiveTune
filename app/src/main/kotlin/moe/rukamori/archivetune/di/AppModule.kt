...
private class LazyCache(
    ...
    private val delegate by lazy(LazyThreadSafetyMode.SYNCHRONIZED) { create() }

    override fun addListener(key: String, listener: Cache.Listener) = delegate.addListener(key, listener)
    override fun removeListener(key: String, listener: Cache.Listener) = delegate.removeListener(key, listener)
    ...
    override fun getCachedLength(key: String, position: Long, length: Long): Long = delegate.getCachedLength(key, position, length)
    override fun getCachedBytes(key: String, position: Long, length: Long): Long = delegate.getCachedBytes(key, position, length)
    override fun applyContentMetadataMutations(key: String, mutations: ContentMetadataMutations) = delegate.applyContentMetadataMutations(key, mutations)
    override fun getContentMetadata(key: String): ContentMetadata = delegate.getContentMetadata(key)
    override fun startReadWrite(key: String, position: Long, length: Long): CacheSpan = delegate.startReadWrite(key, position, length)
    override fun startReadWriteNonBlocking(key: String, position: Long, length: Long): CacheSpan? = delegate.startReadWriteNonBlocking(key, position, length)
    override fun startFile(key: String, position: Long, maxLength: Long): File = delegate.startFile(key, position, maxLength)
    override fun commitFile(file: File, length: Long) = delegate.commitFile(file, length)
    ...
    override fun isCached(key: String, position: Long, length: Long): Boolean = delegate.isCached(key, position, length)
